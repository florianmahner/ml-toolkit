...
class DatasetContainer:
    ...
    test: UnifiedDataset | None


DATASET_REGISTRY: dict[str, Callable[..., UnifiedDataset | DatasetContainer]] = {}


def register_dataset(name: str):
    def decorator(func: Callable[..., UnifiedDataset | DatasetContainer]):
...
def load_dataset(name: str, **kwargs) -> UnifiedDataset:
    if name not in DATASET_REGISTRY:
        raise ValueError(
            f"Dataset '{name}' is not registered. Available: {list(DATASET_REGISTRY.keys())}"
        )
    return DATASET_REGISTRY[name](**kwargs)


def load_datasets(*names: str, **kwargs) -> list[UnifiedDataset | DatasetContainer]:
    """
    Loads multiple datasets by name.
    """
    results = []
    for name in names:
        results.append(load_dataset(name, **kwargs))
    return results


@register_dataset("iris")
def load_iris(root: str, transform: Compose | None = None) -> DatasetContainer:
    iris = sklearn_load_iris()
    ds = UnifiedDataset(iris.data, iris.target)
    return DatasetContainer(name="iris", train=ds, test=None)


@register_dataset("digits")
def load_digits(root: str, transform: Compose | None = None) -> DatasetContainer:
    digits = sklearn_load_digits()
    ds = UnifiedDataset(digits.data, digits.target)
    return DatasetContainer(name="digits", train=ds, test=None)


@register_dataset("mnist")
def load_mnist(root: str, transform: Compose | None = None) -> DatasetContainer:
    ds = MNIST(root, train=True, transform=transform, download=True)
    ds_test = MNIST(root, train=False, transform=transform, download=True)
    return DatasetContainer(name="mnist", train=ds, test=ds_test)


@register_dataset("orl")
def load_orl(root: str, transform: Compose | None = None) -> DatasetContainer:
    ds = ORL(root, transform=transform)
    return DatasetContainer(name="orl", train=ds, test=None)


def load_synthetic_data(
    n_classes=15, samples_per_class=100, noise_std=1.0, random_state=None
):
    rng = np.random.default_rng(random_state)
    # Place centers in a circle
    ...
    for i, center in enumerate(centers):
        # Generate samples around the center with Gaussian noise
        Xi = rng.normal(loc=center, scale=noise_std, size=(samples_per_class, 2))
        X.append(Xi)
...
class ORL(torch.utils.data.Dataset):
    ...
    def _load_data(self):
        file = os.path.join(self.root, "orl", "ORL.mat")
        if not os.path.exists(file):
            raise FileNotFoundError(f"File {file} not found")
        data = loadmat(file)
        self.data = data["data"].reshape(-1, 92, 112)
